...
B_LOGIC = """    var h=+ans[1], m=+ans[2], k=+ans[3], t4=ans[4], w=ans[5];
    ...
    if(ty==='plan'){
      ...
      advice=(LONG?'いまから、使う':'使う')+'教材を、最適な数冊に絞ってください。%(TGB)s入試で出るところは決まっています。必要な範囲を網羅した教材だけを使えば、量に頼らず'+(LONG?'、早く始めるほど合格率は大きく上がります。':'最短で合格点に届きます。');
    } else if(ty==='test'){
    ...
    var C2={plan:'<b>今日やるべき範囲</b>が、一目でわかる',
            lib:'<b>重要度の高い範囲</b>から、出る順で学べる',
            test:'<b>苦手なところだけ</b>、やり直せる',
            log:'<b>苦手な範囲</b>が、一目でわかる！'};
